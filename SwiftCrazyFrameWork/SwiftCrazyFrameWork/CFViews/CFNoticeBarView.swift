...
public enum CFNoticeBarStyle {
    case onStatusBar
    case onNavigationBar
}

public enum CFNoticeBarAnimationType {
    case top
    case bottom
    case left
    case right
...
extension CFNoticeBarAnimationType {
    
    fileprivate func noticeBarViewTransform(with frame: CGRect,  _ style: CFNoticeBarStyle) -> CGAffineTransform {
...
extension CFNoticeBarStyle {
    
   fileprivate func noticeBarProperties() -> CFNoticeBarProperties {
    let screenWidth = UIScreen.main.bounds.width
    var properties: CFNoticeBarProperties
    switch self {
    case .onNavigationBar:
        properties = CFNoticeBarProperties(shadowOffsetY: 3,
    ...
    case .onStatusBar:
        properties = CFNoticeBarProperties(shadowOffsetY: 2,
...
fileprivate struct CFNoticeBarProperties {
...
public struct CFNoticeBarConfig {
    public init() {}
    ...
    public var animationType = CFNoticeBarAnimationType.top
    public var barStyle = CFNoticeBarStyle.onNavigationBar
    
    public init(title: String? = nil,
                image: UIImage? = nil,
                textColor: UIColor = UIColor.white,
                backgroundColor: UIColor = UIColor.orange,
                barStyle: CFNoticeBarStyle = .onNavigationBar,
                animationType: CFNoticeBarAnimationType = .top) {
        self.title = title
        self.image = image
...
open class CFNoticeBarView: UIView {
    
    private var config = CFNoticeBarConfig()
    ...
    public init(config: CFNoticeBarConfig) {
    ...
    private func show(duration: TimeInterval, willShow: () -> Void, completed: ((_ finished: Bool) -> Void)?) {
        ...
        if let subviews = UIApplication.shared.keyWindow?.subviews {
            for view in subviews {
                if view.isKind(of: CFNoticeBarView.self) {
